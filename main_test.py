...
def main() -> None:
    ...
    while True:
        ...
        if user_file_choice == "1":
            ...
            filters_file = financial_transactions(os.path.join(os.path.dirname(__file__), "data", "operations.json"))
            break
        elif user_file_choice == "2":
            print("Выбран CSV-файл.")
            filters_file = read_transaction_csv(os.path.join(os.path.dirname(__file__), "data", "transactions.csv"))
            break
        elif user_file_choice == "3":
            print("Выбран XLSX-файл.")
            filters_file = os.path.join(os.path.dirname(__file__), "data", "transactions_excel.xlsx")
            break
        else:
        ...
        if sort_date == "да":
            while True:
                ...
                if sorting_order == "по возрастанию":
                    filters_file["date"] = False
                    break
                elif sorting_order == "по убыванию":
                    filters_file["date"] = True
                    break
                else:
                    print("Некорректный выбор. Попробуйте еще раз.")
                    continue
    ...
    while True:
        ...
        if user_input == "да":
            ...
            filters_file["description"] = search
            break
        elif user_input == "нет":
            break
        else:
    ...
    transactions = filters_file
    for filter_type, filter_value in filters_file.items():
